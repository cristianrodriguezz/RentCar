...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
    ...
    @Query(value = "SELECT * FROM Producto as p WHERE (p.id not in (SELECT producto_id FROM Producto as p LEFT JOIN Reserva as r ON p.id=r.producto_id WHERE (r.fecha_inicio_reserva BETWEEN ?1 and ?2)  OR (r.fecha_final_reserva  BETWEEN ?1 and ?2))) AND p.ciudad_id LIKE ?3 group by p.id; ",nativeQuery = true)
