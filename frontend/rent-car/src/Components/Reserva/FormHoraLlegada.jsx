      ...
      {({ errors, values }) => (
        <Form className='formulario llegada'>
          ...
          <div className='inter'>
            ...
            !hora 
            ? 
            <p className='seleccionaHorarioLlegada'><CheckCircleOutlineIcon/>Seleccione horario del check-in de su vehiculo</p> 
            : 
            <p className='seleccionaHorarioLlegada'><CheckCircleOutlineIcon/>Tu vehiculo va a estar listo para el check-in entre las {hora} y las {hora}</p>
            }
            <label htmlFor='horario'>Indicá tu horario estimado de llegada</label>
            <Field className='horarioDeLlegadaInput' as="select" id="horario" name="horario" placeholder="email@mail.com" onChange={event => {setHora (event.target.value)}}>
